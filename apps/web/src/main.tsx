...
function ReportPreview({ report, token }: { report: Report; token: string }) {
  ...
  const body = JSON.stringify({ draft: { title: report.title, blocks: (report.blocks || []).map(({ title, content }) => ({ title, content })) } });
  useEffect(() => {
...
function ExportButton({ report, token, draft = false }: { report: Report; token: string; draft?: boolean }) {
  ...
  async function download() {
    ...
    try {
      const pdf = await fetchReportPdf(report.id, token, JSON.stringify(draft ? { draft: { title: report.title, blocks: report.blocks || [] } } : {}));
