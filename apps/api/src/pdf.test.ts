...
test('report preserves order and omits private notes', () => {
  const report = { title: '<Report>', blocks: [{ title: 'First', content: {}, notes: 'SECRET' }, { title: 'Second', content: {} }] };
...
test('Chromium generates a PDF', async () => {
  ...
  if (process.env.PDF_TEST_OUTPUT) {
    const sample = await renderPdf({ title: 'Leadership DNA: Executive Team', blocks: Array.from({ length: 8 }, (_, index) => ({ title: `${index + 1}. Communication and leadership`, content: { type: 'doc', content: [
